...
namespace {
  ...
  // ensures that the engine exits gracefully if the GUI dies
  // unexpectedly.
  ...
  void handle_command(const std::string &command) {
    ...
    if(s == "quit") {
      ...
      std::cout << "id author Luke Boncic" << std::endl;
      print_uci_options();
  ...
  // set_position() is called when the engine receives the "position" UCI
  ...
  // set_option() is called when the engine receives the "setoption" UCI
  // command.  The input parameter is a UCIInputParser.  It is assumed
  // that this parser has consumed the first token of the UCI command
  ...
  // go() is called when the engine receives the "go" UCI command.  The
